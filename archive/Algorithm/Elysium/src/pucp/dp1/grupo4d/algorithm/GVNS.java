...
public class GVNS {
    public static Double D_MIN = 0.001;
    public static Integer I_MAX = 3;
    ...
    public static Integer MAX_INTENTOS = 5;
    ...
    private void solucionInicial(Problematica problematica, Solucion solucion) {
        ...
        for(int i = 0; i < GVNS.I_MAX; i++) {
            ...
            G4D.Logger.logf(">> Iteración: %d de '%d' | Mejor fitness: %.3f%n", i + 1, I_MAX, mejorFitness);
            ...
            sAux.setFitness();
            if(sAux.getFitness() < mejorFitness) {
        ...
        //
        if(!haySolucion) {
            G4D.Logger.logln_err("ERROR TOTAL DE ENRUTAMIENTO");
            this.solucionINI = null;
    ...
    private boolean atenderPedido(Pedido pedido, List<Aeropuerto> origenes, List<Plan> planes,
        ...
        while(!origenesDisponibles.isEmpty()) {
            ...
            if(ruta == null) {
                ...
                if(ruta == null) {
                    G4D.Logger.log_err("ERROR: No es posible generar una ruta a partir de este origen..");
            ...
            if(cantPorEnrutar == 0) {
                ...
                G4D.Logger.logf(">>> PEDIDO #%d ATENDIDO! | '%d' de '%d' productos enrutados!", G4D.Logger.Stats.numPed, G4D.Logger.Stats.posProd, pedido.getCantidadSolicitada());
    ...
    private Ruta construirRutaVoraz(LocalDateTime fechaHoraInicial, LocalDateTime fechaHoraLimite, Aeropuerto origen,
        ...
        if(actual.equals(destino)) {
            G4D.Logger.logln_err(" | ERROR: {ORIG == DEST}");
            return null;
